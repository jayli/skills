...
ISSUES_FILE="${SCRIPT_DIR}/../../.ruby_unused_issues.txt"
> "$ISSUES_FILE"
...
check_ruby_unused() {
    ...
    while IFS= read -r line; do
        ...
        add_issue "P2" "$file" "$lineno" "可能存在未使用的require" "$content" "检查并删除"
        unused_imports=$((unused_imports + 1))
    done < <(grep -rnE "^require |^require_relative " --include="*.rb" . 2>/dev/null | head -20)
    ...
    while IFS= read -r line; do
        ...
        # 排除以 _ 开头的变量
        if ! echo "$content" | grep -qE "^\s*_"; then
            add_issue "P2" "$file" "$lineno" "变量定义后可能未使用" "$content" "删除或使用该变量"
            unused_vars=$((unused_vars + 1))
        fi
    done < <(grep -rnE "^\s*[a-z_][a-zA-Z0-9_]*\s*=\s*[^=]" --include="*.rb" . 2>/dev/null | \
        grep -v "@@\|@\|\$" | head -15)
    ...
    done < <(grep -rnE "^\s*#.*def |^\s*#.*require |^\s*#.*class |^\s*#.*module " --include="*.rb" . 2>/dev/null | head -15)
