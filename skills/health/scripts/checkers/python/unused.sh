...
# 问题详情输出文件
ISSUES_FILE="${SCRIPT_DIR}/../../.python_unused_issues.txt"
> "$ISSUES_FILE"
...
check_python_unused() {
    ...
    while IFS= read -r line; do
        ...
        add_issue "P2" "$file" "$lineno" "可能存在未使用的导入" "$content" "检查并删除未使用的导入"
        unused_imports=$((unused_imports + 1))
    done < <(grep -rnE "^import |^from " --include="*.py" . 2>/dev/null | grep -v "as\|#" | head -20)
    ...
    done < <(grep -rnE "^\s*\w+\s*=\s*[^=]" --include="*.py" . 2>/dev/null | grep -v "return\|print\|yield\|raise" | head -15)
    ...
    done < <(grep -rnE "^\s*#.*def |^\s*#.*import |^\s*#.*class " --include="*.py" . 2>/dev/null | head -15)
