...
# 问题详情输出文件
ISSUES_FILE="${SCRIPT_DIR}/../../.python_security_issues.txt"
> "$ISSUES_FILE"
...
check_python_security() {
    ...
    while IFS= read -r file; do
        ...
        secrets_found=$((secrets_found + 1))
    done < <(find . -name "*.env" -o -name "config.py" -o -name "settings.py" 2>/dev/null | \
    ...
    done < <(grep -rnE "execute.*%s|execute.*\+|cursor\.execute.*f\"|cursor\.execute.*\.format" \
    ...
    if [ -f "requirements.txt" ]; then
        ...
        while IFS= read -r line; do
            ...
            if ! echo "$line" | grep -qE "==|>=|<=|~=|!="; then
                local lineno=$(grep -n "$line" requirements.txt | head -1 | cut -d: -f1 || echo "N/A")
                add_issue "P1" "requirements.txt" "$lineno" "依赖未固定版本" "$line" "指定具体版本号"
                vulns=$((vulns + 1))
            fi
        done < <(cat requirements.txt 2>/dev/null | head -15)
    fi
